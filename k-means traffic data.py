...
kmeans = KMeans(n_clusters=7, n_init=10)  # Define the number of clusters
...
y = Result_3D[:, 1]
z = Result_3D[:, 2]
shifted_labels = df_Label - 1
unique_labels = np.unique(df_Label)
cmap = plt.get_cmap("Set1", len(unique_labels))  # Use a categorical color map
norm = colors.BoundaryNorm(np.arange(len(unique_labels) + 1), cmap.N)  # Normalize the color map
sc3d2 = ax2.scatter(x, y, z, cmap=cmap, edgecolor='k', s=200, c=shifted_labels, norm=norm)
ax2.set_xlabel('PCA 1')
ax2.set_ylabel('PCA 2')
ax2.set_zlabel('PCA 3')
ax2.set_title('Day of Week Labels on 3D PCA Projection')
cbar = plt.colorbar(sc3d2, label='Day of Week', ticks=np.arange(len(unique_labels)))
cbar.ax.set_yticklabels(np.arange(1, len(unique_labels) + 1))  # Adjust color bar ticks to show 1 to 7
